  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-3">
        ...
        <p className="text-slate-400 text-sm font-semibold">Loading internships records...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">Submit details and documents for industrial internship verifications.</p>
        </div>
        <button
      ...
      {records.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-slate-900/20 border border-dashed border-slate-800 rounded-2xl">
          <Briefcase className="h-16 w-16 text-slate-700 mb-3 animate-pulse" />
          <h3 className="text-base font-bold text-white">No internships recorded yet</h3>
          <p className="text-slate-500 text-xs mt-2 max-w-sm">Provide details of software development, product, or research internships you have completed.</p>
          <button
            onClick={openAddModal}
                ...
                <div>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="font-extrabold text-white text-lg mt-2 truncate max-w-xs">{r.role}</h3>
                      <p className="text-slate-550 text-[10px] mt-1 font-semibold">{r.company}</p>
                    </div>
                  ...
                  <div className="mt-4 text-[10px] text-slate-500 font-semibold uppercase tracking-wider">
                    Duration: <strong className="text-slate-400">{r.startDate.split('T')[0]} to {r.endDate.split('T')[0]}</strong>
                  </div>
          ...
          <div className="relative bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Role / Position</label>
                  <input
                    type="text"
                    required
                    value={role}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Internship Type</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Technologies Used</label>
                  <input
                    type="text"
                    value={technologies}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Description of work</label>
                <textarea
                  required
                  rows={3}
                  value={description}

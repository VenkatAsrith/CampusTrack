  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-3">
        ...
        <p className="text-[#6C757D] text-sm font-semibold">Loading certifications...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-extrabold text-[#1E1E1E]">Certifications</h1>
          <p className="text-[#6C757D] text-xs mt-1">Manage certifications from AWS, GCP, Coursera, Meta, etc.</p>
        </div>
        <button
      ...
      {records.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-white border border-dashed border-[#CBD5E1] rounded-2xl">
          <Award className="h-14 w-14 text-[#94A3B8] mb-3" />
          <h3 className="text-base font-bold text-[#1E1E1E]">No certifications recorded yet</h3>
          <p className="text-[#6C757D] text-xs mt-1.5 max-w-sm">Showcase your specialized tech knowledge by adding your industry-standard cloud, cybersecurity, coding, or AI certifications.</p>
          <button
            onClick={openAddModal}
                ...
                <div>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="font-extrabold text-[#1E1E1E] text-base mt-2 truncate max-w-xs">{r.certificationName}</h3>
                      <p className="text-[#6C757D] text-xs mt-0.5 font-semibold">{r.issuingOrganization}</p>
                    </div>
                    <StatusBadge status={r.verification.status} />
                  </div>

                  <div className="mt-4 space-y-1.5 text-xs text-[#6C757D]">
                    {r.credentialId && <p>ID: <strong className="font-mono text-[#1E1E1E]">{r.credentialId}</strong></p>}
          ...
          <div className="relative bg-white border border-[#E5E9F2] rounded-2xl p-6 w-full max-w-lg shadow-2xl overflow-y-auto max-h-[90vh] my-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1E1E1E] uppercase tracking-wider mb-1">Certification Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1E1E1E] uppercase tracking-wider mb-1">Issuing Organization *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1E1E1E] uppercase tracking-wider mb-1">Issue Date *</label>
                  <input
                    type="date"
                    required
                    value={issueDate}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1E1E1E] uppercase tracking-wider mb-1">Credential ID (optional)</label>
                  <input
                    type="text"
                    value={credentialId}
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1E1E1E] uppercase tracking-wider mb-1">Credential URL (optional)</label>
                  <input
                    type="url"
                    value={credentialUrl}

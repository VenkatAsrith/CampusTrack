...
const StudentLayout: React.FC = () => {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    return localStorage.getItem('campustrack_student_sidebar_collapsed') === 'true';
  });
  ...
  const handleToggleCollapse = () => {
    setSidebarCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem('campustrack_student_sidebar_collapsed', String(next));
      return next;
    });
  };
  ...
  return (
    <div className="relative flex h-screen bg-[#F4F6FA] overflow-hidden text-[#1E1E1E] font-sans select-none">
      {/* Desktop Collapsible Sidebar (fixed on left) */}
      <div 
      ...
      <div className="relative z-10 flex-1 flex flex-col h-full overflow-hidden min-w-0">
        <TopNavbar 
          title="CampusTrack Student Portal" 
